...
function driveStatusLabel(status: string): { text: string; color: string } {
    if (status === 'uploaded') return { text: '✓ Uploaded to Drive', color: 'var(--success)' }
    if (status === 'not_configured') return { text: '— Drive not configured', color: 'var(--t3)' }
    if (status === 'libraries_missing') return { text: '⚠ Drive libraries missing', color: 'var(--warning)' }
    if (status.startsWith('upload_failed')) return { text: '✗ Drive upload failed', color: 'var(--danger)' }
    return { text: status, color: 'var(--t3)' }
}
    ...
    async function handleSaveDrive() {
        ...
        try {
            ...
            const data = await apiFetch<{ success: boolean; message: string; service_account_email: string }>(
                '/api/backups/drive/configure/', { method: 'POST', body: JSON.stringify(body) }
            )
        ...
        <div style={{ maxWidth: '720px', animation: 'fadeIn 0.25s ease' }}>
            ...
            <div className="card" style={{ marginBottom: '24px' }}>
                ...
                <div style={{ padding: '20px' }}>
                    ...
                    >
                        {creating ? (
                            <>
                                <span className="spinner" style={{ width: '12px', height: '12px', marginRight: '6px' }} /> Creating backup…
                            </>
                        ) : (
                            'Create Backup Now'
                        )}
                    ...
                    {createError && (
                        <div className="ferr" style={{ marginTop: '12px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                    ...
                    {lastResult && (
                        ...
                        >
                            <p style={{ fontSize: '13px', margin: '4px 0', color: 'var(--t1)' }}>
                                <strong style={{ color: 'var(--t2)' }}>File:</strong> {lastResult.filename}
                            </p>
                            <p style={{ fontSize: '13px', margin: '4px 0', color: 'var(--t1)' }}>
                                <strong style={{ color: 'var(--t2)' }}>Size:</strong> {lastResult.size_readable}
                            </p>
                            <p style={{ fontSize: '13px', margin: '4px 0', color: 'var(--t1)' }}>
                                <strong style={{ color: 'var(--t2)' }}>Created:</strong> {lastResult.created_at}
                            </p>
                            {lastResult.drive_status && (() => {
                                const { text, color } = driveStatusLabel(lastResult.drive_status)
                                return (
                                    <p style={{ fontSize: '13px', margin: '4px 0', color }}>
                                        <strong style={{ color: 'var(--t2)' }}>Drive:</strong>{' '}
                                        {text}
                                        {lastResult.drive_link && (
                                            <a href={lastResult.drive_link} target="_blank" rel="noreferrer" style={{ color: 'var(--primary)', textDecoration: 'none', marginLeft: '6px', fontWeight: 600 }}>
                                                Open ↗
                                            </a>
                                        )}
                                    </p>
            ...
            <div className="card" style={{ marginBottom: '24px' }}>
                ...
                <div style={{ padding: '20px' }}>
                    {driveInfo?.configured && (
                        ...
                        >
                            <div style={{ fontSize: '12.5px', margin: '4px 0', display: 'flex', gap: '8px' }}>
                                <span style={{ color: '#059669', fontWeight: 600, minWidth: '120px' }}>Service Account</span>
                                <span style={{ color: 'var(--t1)', wordBreak: 'break-all' }}>{driveInfo.service_account_email}</span>
                            </div>
                            <div style={{ fontSize: '12.5px', margin: '4px 0', display: 'flex', gap: '8px' }}>
                    ...
                    {driveMsg && (
                        <p style={{ fontSize: '13px', marginTop: '12px', color: driveMsgType === 'ok' ? '#059669' : 'var(--danger)', fontWeight: 500 }}>
                            {driveMsgType === 'ok' ? '✓' : '✗'} {driveMsg}
                        </p>
                    )}

                    {/* Setup guide */}
                    <details style={{ marginTop: '20px', borderTop: '1px solid var(--border)', paddingTop: '16px' }}>
                        <summary style={{ fontSize: '13px', fontWeight: 600, color: 'var(--t2)', cursor: 'pointer', userSelect: 'none' }}>
                            How to set up Google Drive (first time)
                        </summary>
                        <ol style={{ fontSize: '13px', color: 'var(--t2)', lineHeight: '1.8', paddingLeft: '20px', marginTop: '8px' }}>
                            ...
                            <li>Click the service account → <strong>Keys → Add Key → JSON</strong>. Download the file.</li>
                            <li>Open <strong>Google Drive</strong> → create or pick a folder for backups.</li>
                            <li>Right-click the folder → <strong>Share</strong> → paste the service account email (ends in <em>@…gserviceaccount.com</em>) → give <strong>Editor</strong> access.</li>
            ...
            <div className="card">
                ...
                <div style={{ padding: '20px' }}>
                    {backupDir && (
                        <p style={{ fontSize: '12px', color: 'var(--t3)', marginBottom: '12px', fontFamily: 'monospace', wordBreak: 'break-all' }}>
                            Stored in: {backupDir}
                        </p>
                    )}
                    ...
                    {loadingList ? (
                        <div style={{ textAlign: 'center', padding: '20px', color: 'var(--t3)' }}>
                            <span className="spinner" style={{ width: '16px', height: '16px', display: 'inline-block' }} /> Loading backups…
                        </div>
                    ) : backups.length === 0 ? (
                                ...
                                <tbody>
                                    {backups.map((b) => (
                                        <tr key={b.filename}>
                                            ...
                                            <td style={{ textAlign: 'right', color: 'var(--t2)' }}>{b.size_readable}</td>
                                            <td style={{ textAlign: 'right', color: 'var(--t2)' }}>{b.created_at}</td>
